   ...
   public static NewsAbstract parseJsonObject(JSONObject jsonObject) {
      ...
      try {
         ...
         if (!jsonObject.isNull(JSON_KEY_COLOR)) {
            result.setColor(NewsAbstract.parseColorValue(jsonObject
                  .getString(JSON_KEY_COLOR).trim()));
         }
         if (!jsonObject.isNull(JSON_KEY_PUBLISHED_TIME)) {
            result.setPublishedTime(jsonObject.getString(JSON_KEY_PUBLISHED_TIME)
                  .trim());
         }
         ...
         if (!jsonObject.isNull(JSON_KEY_TITLE)) {
            result.setTitle(jsonObject.getString(JSON_KEY_TITLE)
                  .trim());
         }
